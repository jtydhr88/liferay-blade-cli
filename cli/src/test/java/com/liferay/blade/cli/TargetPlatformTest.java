...
public class TargetPlatformTest {
	...
	@Test
	public void testSettingNotSupportTargetPlatform() throws Exception {
		_targetPlatformSettingConfigure(false);

		String[] args =
			{"--base", _gradleWorkspaceDir.getAbsolutePath(), "create", "-t", "activator", "not-support-activator"};

		new BladeTest().run(args);

		File modules = new File(_gradleWorkspaceDir, "modules");

		File project = new File(modules, "not-support-activator");
	...
	@Test
	public void testWithoutWithoutWorkspace() throws Exception {
		String[] args =

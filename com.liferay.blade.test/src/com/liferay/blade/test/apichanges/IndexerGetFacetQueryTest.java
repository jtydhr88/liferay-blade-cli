...
public class IndexerGetFacetQueryTest extends BaseAPIChangesTest {
	...
	public void assetEntriesFacetFile() throws Exception {
		...
		for (ServiceReference<FileMigrator> fm : fileMigrators) {
			...
			if (fmigrator instanceof IndexerGetFacetQuery) {
				problems = fmigrator.analyze(assetEntriesFacetFile);
			}
	...
	public void indexerWrapperFile() throws Exception {
		...
		for (ServiceReference<FileMigrator> fm : fileMigrators) {
			...
			if (fmigrator instanceof IndexerGetFacetQuery) {
				problems = fmigrator.analyze(indexerWrapper);
			}
